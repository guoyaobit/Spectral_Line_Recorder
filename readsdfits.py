...
raw = data["DATA"]    
# dcol = data['DATA'][0]
tdim = hdul[1].header['TDIM20']
shape = eval(tdim)
print(shape)
spectrum = raw[-1].reshape(shape)
spectrum_stokes = spectrum[:, 0, :, 0, 0]
stokes_I = 10*np.log10(spectrum_stokes[:, 0]+1e-10)
...
plt.title("Stokes I Spectrum")
plt.show()

# print("Raw DATA shape:", dcol.shape)
# print(dcol)
